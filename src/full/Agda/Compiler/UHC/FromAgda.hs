  ...
  case theDef defini of
    ...
    (Axiom{})
      | Just imp <- getFFIFunImport Target_UHC_Native compRep -> do -- Axioms get their code from COMPILED_UHC pragmas
        ...
        ffiCall <- case imp of
              FunImp_UHC_HS x -> do
                wrapper <- lift $ dropSetLevelArgs (defType defini) (C.TDef fQnm)
                withNames [(fQnm, x)] $ runTT $ compileTerm wrapper
              FunImp_UHC_Core x -> do
                case coreExprToCExpr x of
