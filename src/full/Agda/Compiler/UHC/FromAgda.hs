...
translateDefn (n, defini) = do
  ...
  let compRep = defCompiledRep defini
  ...
  case theDef defini of
    ...
    (Constructor{}) | Nothing <- getFFIConBind Way_UHC_Core compRep -> do -- become functions returning a constructor with their tag
    ...
    r@(Record{}) -> do
        ...
        return [mkBind1 crName (mkLam vars $ mkUnit opts)]
    (Axiom{})
      | Just imp <- getFFIFunImport Target_UHC_Native compRep -> do -- Axioms get their code from COMPILED_UHC pragmas
        -- create an additional def with the ffi call
        let (x, mkWrapper) = case imp of
              FunImp_UHC_HS x   -> (x, dropSetLevelArgs (defType defini))
        ...
        let ffiFun = mkBind1 fCnm x'
            wrapperFun = mkBind1 crName wrapper
        return [ffiFun, wrapperFun]
            
    (Axiom{})
      | otherwise -> do
        addExports [crName]
        return [mkBind1 crName
